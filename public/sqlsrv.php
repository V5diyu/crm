...
$sql_1 = "select p.cus_os_no as contract,c.NAME as customer,SUM(tp.AMT) as totalprice, min(s.Name) as salesman, min(p.OS_DD) as orderdate ,min(p.Modify_dd) as modifydate from MF_POS p left join CUST c on p.cus_no =c.cus_no left join TF_POS tp on tp.os_no = p.os_no left join SALM s on s.sal_no = p.sal_no where ((p.OS_DD >='" . $str_pre . "' AND p.OS_DD<'" . $str_current. "') or (p.Modify_dd >='". $str_pre ."' AND p.Modify_dd<'" . $str_current . "')) and p.cus_os_no is not null GROUP BY p.cus_os_no,c.NAME";

$sql_2 = "select p.cus_os_no as contract, ((select SUM(ts.AMT) from TF_PSS ts where ts.cus_os_no = p.cus_os_no)/(select SUM(tp.AMT) from TF_POS tp where tp.cus_os_no = p.cus_os_no)) as delivery,  p.ps_dd as deliverydate from MF_PSS p where  ((p.ps_dd >='". $str_pre ."' AND p.ps_dd<'". $str_current ."') or (p.Modify_dd >='". $str_pre ."' AND p.Modify_dd<'". $str_current ."')) and p.cus_os_no is not null and p.cus_os_no <>''";

$sql_3 = "select tm.CAS_NO  as contract, tm.amtn_bb as paid  from  TF_MON tm  where (tm.rp_DD >='" . $str_pre ."' AND tm.rp_DD<'". $str_current ."') or (tm.Modify_dd >='". $str_pre ."' AND tm.Modify_dd<'" . $str_current . "')";
...
$sql_pay_detail = "select tm.CAS_NO  as contract,  tc.amtn_cls as payment, tc.RP_DD as paydate  from  TF_MON tm  ,TC_MON tc  where tc.RP_NO = tm.RP_NO and tc.rp_dd >='" . $str_pre . "' AND tc.rp_dd<'" . $str_current . "'";
...
while($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)){
    ...
    $arr_order_date = (array)$row['orderdate'];
    $arr_modify_date = (array)$row['modifydate'];
    $ret_1[$row['contract']]['contractTime']  = $arr_order_date['date'];
    $ret_1[$row['contract']]['modifyTime']    = $arr_modify_date['date'];
    unset($arr_order_date);
    unset($arr_modify_date);
    unset($row);
}
...
while ($row = sqlsrv_fetch_array($stmt,SQLSRV_FETCH_ASSOC)) {
    ...
    $data_log[$id]['syn_field'] = (string)json_encode(['订单号'=>$row['contract'],'付款金额'=>$row['payment']],JSON_UNESCAPED_UNICODE);
...
if (!empty($ret_1)) {
    foreach ($ret_1 as $k => $v) {
        ...
        $item         = [
            ...
            'K_fkbl'     => (empty($v['paid1']) ? 0: strEmptyFloat($v['paid1'])) / strEmptyFloat($v['totalprice']),         //付款比例
            'L_fkxq'     => '',                                                                                             //付款详情
        ...
        //(公司数据统计)判断合同的年月是否在数组中，没有则添加下标以及初始化数据，有相应的键值，则对应的总价进行增加更新
        //定时任务的处理，和一次同步的处理需要区别！！！


        /*$year_mon = date('Y-m',strtotime(empty($v['modifyTime'] ? $v['contractTime'] : $v['modifyTime'])));*/
...
if (!empty($company_arr)) {
    foreach ($company_arr as $key_company => $value_company) {
        ...
        echo $time_start . '<br >';
        echo $time_end .  '<br >';
        //$mod_data->get(['time' => $value_company]);
    }
}
